...
class twilio_responder_Public {
	...
	if(str_contains($msg, $keyword1)){
			...
			$text = "Your order has been confirmed for shipping. If it is after 3PM MT, your order will be sent on the next available shipping day(Mon-Wed)";
			$this->send_twilio_text($phone, $text);
			return 'Confirmation text sent';
	    } 
			...
			$text = "Please provide the shipping address or any additional details.";    
			$note = $order->get_customer_note(); 
			$note = $note . PHP_EOL . $msg;
			$order->set_customer_note( $note );   
			$order->save();
